...
(async () => {
  ...
  try {
    ...
    if (payload && payload["P-9"] && payload["P-9"].state) p9State = payload["P-9"].state;
    else if (payload && payload["P-9"]) p9State = payload["P-9"];
  } catch (err) {
    console.error("Could not read well-presets.json:", err && err.message ? err.message : err);
    process.exit(2);
  }
  if (!p9State) {
  ...
  try {
    await page.goto(indexPath, { waitUntil: "domcontentloaded", timeout: 10000 });
    await page.waitForFunction(() => !!document.getElementById("preset_list"), { timeout: 5000 });
    ...
    console.error("ERROR running smoke test", err && err.message ? err.message : err);
    await browser.close();
    process.exit(4);
  }
})();